...
public class ShockwaveSphereEntityRenderer extends EntityRenderer<ShockwaveSphereEntity> {
    ...
    public void render(ShockwaveSphereEntity entity, float yaw, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumers, int light) {
        ...
        matrixStack.push();
        matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(alpha / 2 * 180));
        renderSphere(matrixStack, vertexConsumer, MathHelper.clamp(alpha - 0.2f, 0, 1), entity.getCurrentRadius());
        matrixStack.pop();

        matrixStack.push();
        matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(-alpha / 1.5f * 180));
        renderSphere(matrixStack, vertexConsumer, MathHelper.clamp(alpha/1.5f - 0.1f, 0, 1), entity.getCurrentRadius()/1.5f);
        matrixStack.pop();
        
        matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(alpha * 180));
    ...
    private void renderSphere(MatrixStack matrixStack, VertexConsumer vertexConsumer, float alpha, float radius) {
        ...
        for (int i = 0; i < SPHERE_ROWS; i++) {
            ...
            for (int j = 0; j < SPHERE_ROWS; j++) {
                ...
                Vec3f v0 = getSphereDot(minPhi, minTheta, radius);
                Vec3f v1 = getSphereDot(minPhi, maxTheta, radius);
                Vec3f v2 = getSphereDot(maxPhi, maxTheta, radius);
                Vec3f v3 = getSphereDot(maxPhi, minTheta, radius);
    ...
    private Vec3f getSphereDot(float phi, float theta, float radius) {
        float x = (float) (Math.sin(phi) * Math.cos(theta));
        float y = (float) Math.cos(phi);
        float z = (float) (Math.sin(phi) * Math.sin(theta));
        Vec3f vec3f = new Vec3f(x, y, z);
        vec3f.multiplyComponentwise(radius, radius, radius);
        return vec3f;
    }
}
