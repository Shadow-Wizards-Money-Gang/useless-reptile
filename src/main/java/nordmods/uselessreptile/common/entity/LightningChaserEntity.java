...
public class LightningChaserEntity extends URRideableFlyingDragonEntity implements MultipartEntity {
    ...
    public void shoot() {
        ...
        for (int i = 1; i <= LightningBreathEntity.MAX_LENGTH; i++) {
            ...
            for (VoxelShape ignored : getWorld().getBlockCollisions(lightningBreathEntity, lightningBreathEntity.getBoundingBox())) {
                blockCollision = true;
                break;
            }
            boolean collides = blockCollision ||
    ...
    public void updateChildParts() {
        ...
        if (isFlying()) {
            if (isMoving() && !isMovingBackwards() && !isSecondaryAttack()) {
                if (getTiltState() == 2) {
                    wing1LeftPos = new Vec3f(wing1Left.getWidth() + 0.5f, 0, 0.5f);
                    wing1LeftScale = new Vec2f(1, 1.5f);

                    wing2LeftPos = new Vec3f(wing1Left.getWidth() + 0.5f, 0, -0.5f);
                    wing2LeftScale = new Vec2f(1, 1.5f);

                    wing1RightPos = new Vec3f(-wing1Right.getWidth() - 0.5f, 0, 0.5f);
                    wing1RightScale = new Vec2f(1, 1.5f);

                    wing2RightPos = new Vec3f(-wing1Right.getWidth() - 0.5f, 0, -0.5f);
                    wing2RightScale = new Vec2f(1, 1.5f);
                } else {
                    wing1LeftPos = new Vec3f(wing1Left.getWidth(), 0, 0);
                    wing1LeftScale = new Vec2f(1, 2.5f);

                    wing2LeftPos = new Vec3f(wing1Left.getWidth() * 2, 0, 0);
                    wing2LeftScale = new Vec2f(1, 2.5f);

                    wing1RightPos = new Vec3f(-wing1Right.getWidth(), 0, 0);
                    wing1RightScale = new Vec2f(1, 2.5f);

                    wing2RightPos = new Vec3f(-wing1Right.getWidth() * 2, 0, 0);
                    wing2RightScale = new Vec2f(1, 2.5f);
                }
                neck1Pos = new Vec3f(yawOffset * 0.25f, pitchOffset * 0.75f, 2f);
                neck2Pos = new Vec3f(yawOffset * 0.75f, pitchOffset * 1, 2.75f - Math.abs(yawOffset) * 0.25f);
                headPos = new Vec3f(yawOffset * 1.5f, pitchOffset * 1.25f, 3.5f - Math.abs(yawOffset) * 0.5f);

                tail1Pos = new Vec3f(yawOffset * 0.25f, -pitchOffset * 1, -2);
                tail2Pos = new Vec3f(yawOffset * 0.5f, -pitchOffset * 1.25f, -3);
                tail3Pos = new Vec3f(yawOffset * 1.25f, -pitchOffset * 1.5f , -4 + Math.abs(yawOffset) * 0.25f);
            } else {
                wing1LeftPos = new Vec3f(wing1Left.getWidth(), getHeight()/4, -0.5f);
                wing1LeftScale = new Vec2f(getHeight()/2, 3);

                wing2LeftPos = new Vec3f(wing1Left.getWidth() * 1.75f, getHeight()/4, -1);
                wing2LeftScale = new Vec2f(getHeight()/2, 2);

                wing1RightPos = new Vec3f(-wing1Right.getWidth(), getHeight()/4, -0.5f);
                wing1RightScale = new Vec2f(getHeight()/2, 3);

                wing2RightPos = new Vec3f(-wing1Right.getWidth() * 1.75f, getHeight()/4, -1);
                wing2RightScale = new Vec2f(getHeight()/2, 2);

                neck1Pos = new Vec3f(0, getHeight(), 1);
                neck2Pos = new Vec3f(yawOffset * 0.5f, getHeight(), 1.5f);
                headPos = new Vec3f(yawOffset,  getHeight() + 0.1f, 2f);

                tail1Pos = new Vec3f(yawOffset * 0.5f, getHeight() - 3.5f, -2);
                tail2Pos = new Vec3f(yawOffset * 1.25f, getHeight() - 4.5f, -2.25f);
                tail3Pos = new Vec3f(yawOffset * 2f, getHeight() - 5.5f , -2.5f);
            }
        } else {
            if (getIsSitting()) {
                wing1LeftPos = new Vec3f(getWidth() / 2, 0, 0.5f);
                wing1LeftScale = new Vec2f(getHeight() - 1, getWidth() / 2f);

                wing2LeftPos = new Vec3f(getWidth() / 2 + 0.25f, 0.75f, -0.5f);
                wing2LeftScale = new Vec2f(getHeight() - 1.5f, getWidth() / 2f);

                wing1RightPos = new Vec3f(-getWidth() / 2, 0, 0.5f);
                wing1RightScale = new Vec2f(getHeight() - 1, getWidth() / 2f);

                wing2RightPos = new Vec3f(-getWidth() / 2 - 0.25f, 0.75f, -0.5f);
                wing2RightScale = new Vec2f(getHeight() - 1.5f, getWidth() / 2f);

                if (hasSurrendered()) {
                    neck1Pos = new Vec3f(0, 1.6f, 1);
                    neck2Pos = new Vec3f(yawOffset * 0.4f, 1.3f, 1.7f);
                    headPos = new Vec3f(yawOffset * 0.8f,  0.5f, 2.4f);
                } else {
                    neck1Pos = new Vec3f(0, getHeight() - 0.5f, 1);
                    neck2Pos = new Vec3f(yawOffset * 0.4f, getHeight() - 0.2f, 1.5f);
                    headPos = new Vec3f(yawOffset * 0.8f, getHeight() + 0.1f, 2f);
                }

                tail1Pos = new Vec3f(0, 0.3f, -getWidth() + 0.8f);
                tail2Pos = new Vec3f(0, 0.35f, -getWidth() - 0.2f);
                tail3Pos = new Vec3f(0, 0.4f , -getWidth() - 1.2f);
                ...
                neck1Pos = new Vec3f(0, getHeight() - 1f, 1);
                neck2Pos = new Vec3f(yawOffset * 0.4f, getHeight() - 0.75f, 1.5f);
                headPos = new Vec3f(yawOffset * 0.8f,  getHeight() - 0.4f, 2f);

                tail1Pos = new Vec3f(yawOffset * 0.2f, getHeight() - 1.5f, -getWidth() + 0.9f);
                tail2Pos = new Vec3f(yawOffset * 0.4f, getHeight() - 0.8f,  -getWidth() + 0.2f);
                tail3Pos = new Vec3f(yawOffset * 0.8f, getHeight() - 0.5f , -getWidth() - 0.7f);
            }
        }
