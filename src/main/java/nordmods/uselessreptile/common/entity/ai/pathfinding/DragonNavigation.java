...
public class DragonNavigation extends MobNavigation {
    ...
    public void tick() {
        ...
        if (!isIdle() && target != null) {
            ...
            if (!isIdle()) {
                ...
                if (yDiffNode > entity.stepHeight && entity.horizontalCollision) entity.getJumpControl().setActive();
            }
        }
    }
    ...
    protected void continueFollowingPath() {
        ...
        boolean bl = d < (double)nodeReachProximity && f < (double)nodeReachProximity &&  yDiff <= entity.stepHeight && yDiff > -3.0D;

        if (bl || entity.canJumpToNextPathNode(currentPath.getNode(index).type) && shouldJumpToNextNode(vec3d)) currentPath.next();
