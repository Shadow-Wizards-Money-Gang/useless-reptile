...
public class LightningBreathEntity extends ProjectileEntity {
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (target.damage(DamageSource.LIGHTNING_BOLT, 6f))
    ...
    public void tick() {
        ...
        if (++age <= MAX_AGE) {
            ...
            for (VoxelShape ignored : getWorld().getBlockCollisions(this, getBoundingBox())) {
                blockCollision = true;
                break;
            }
            ...
            for (BlockPos blockPos : blocks) {
                ...
                PlayerEntity rider = getOwner() instanceof URRideableDragonEntity dragon && dragon.canBeControlledByRider() ?
                        (PlayerEntity) getPrimaryPassenger() : null;
            ...
            sorted.forEach(fallingBlockEntity -> {
                Vec3d velocity = Vec3d.ofCenter(getBlockPos().subtract(fallingBlockEntity.getBlockPos()));
                velocity.add(0, 1, 0);
    ...
    private boolean canTarget(Entity target) {
        if (target.isInvulnerableTo(DamageSource.LIGHTNING_BOLT)) return false;
    ...
    public static class LightningBreathBolt {
        ...
        public record Segment (Vec3d startPoint, Vec3d endPoint) {}
    }
}
